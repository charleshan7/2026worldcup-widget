...
4. If configuration is missing, copy `Config.local.xcconfig.example` to `Config.local.xcconfig` and ask the user to paste their own football-data.org token locally. Do not request that they send the token in chat.
...
- Keep `Config.local.xcconfig` ignored by Git.
